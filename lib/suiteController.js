...
module.exports = {
    ...
    createSuite(options) {
        ...
        global.gemini.suite(options.suiteName, suite => {
            ...
            if (options.auth && options.capture) {
                suite.before(createAuthHook(options));
                _.remove(inheretedOptions, opt => opt === 'auth');
            }
...
function createAuthHook(options) {
    assert(options.auth.login, 'auth.login should be non-empty');
    assert(_.isString(options.auth.login), 'auth.login should have string type');
    assert(options.auth.password, 'auth.password should be non-empty');
    assert(_.isString(options.auth.password), 'auth.password should have string type');

    let beforeAuthHook = function(actions, find) {
        actions
            .open(authRequest(options.auth.login, options.auth.password))
            .open(fullUrl(options.topUrl, options.url));
    };

    return beforeAuthHook;
}
...
function fullUrl(rootUrl, suiteUrl) {
 ...
 * @property {Object} [auth] Авторизационная информация.
 * @property {Object} [auth.login] Логин для авторизации перед выполнением сьюта.
 * @property {Object} [auth.password] Пароль для авторизации перед выполнением сьюта.
